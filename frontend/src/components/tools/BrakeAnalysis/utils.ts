import { BrakeAnalysisPreset } from './types';
import { SaveFileDialog, WriteFile } from '../../../../wailsjs/go/main/App';

export const loadPresets = (): BrakeAnalysisPreset[] => {
  const saved = localStorage.getItem('brakeAnalysisPresets');
  if (saved) {
    try {
...
export const savePresets = (presets: BrakeAnalysisPreset[]): void => {
  localStorage.setItem('brakeAnalysisPresets', JSON.stringify(presets));
};
...
export const exportToPNG = async (
  ...
  try {
    const filePath = await SaveFileDialog('brake_analysis.png');
    if (!filePath) return;
