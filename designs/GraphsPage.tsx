  ...
  return (
    <div ref={ref} style={{
      ...
      <div style={{ display: 'flex', borderBottom: `2px solid ${GOLD}` }}>
        ...
        <button style={tabBtnStyle(tab === 'multi')}  onClick={() => setTab('multi')}>📂 Multi-File / MRTF</button>
      </div>
        ...
        {tab === 'single' ? (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            <p style={{ fontSize: 12, color: '#aaa', margin: 0 }}>
              Load a single <strong style={{ color: 'white' }}>BTF</strong> or <strong style={{ color: 'white' }}>MRTF</strong> data file.
            </p>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            <p style={{ fontSize: 12, color: '#aaa', margin: 0 }}>
              Combine multiple lap files or load a saved <strong style={{ color: 'white' }}>MRTF</strong> multi-run file.
            </p>
            {/* Action buttons */}
...
const GraphsPage: React.FC = () => {
  ...
  // File name display (just the basename)
  ...
  return (
    <div style={{ height: '100vh', width: '100vw', backgroundColor: BG_PAGE, color: 'white', display: 'flex', flexDirection: 'column', overflow: 'hidden', position: 'fixed', top: 0, left: 0, fontFamily: FONT }}>
      ...
      <div style={{ backgroundColor: BG_CONTROL, padding: '0 12px', borderBottom: `2px solid ${GOLD}`, display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0, height: 48 }}>
        ...
        {/* Spacer — pushes edit group right */}
        <div style={{ flex: 1 }} />

        {/* Separator */}
        <div style={{ width: 1, alignSelf: 'stretch', background: 'rgba(255,255,255,0.1)', margin: '8px 2px' }} />

        {/* Edit group — far right */}
        <button
      ...
      {/* ── Graph Area ───────────────────────────────────────── */}
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0, padding: '12px 12px 0', overflow: 'hidden', gap: 8, boxSizing: 'border-box' }}>
      ...
      {/* ── Preset Bar (bottom) ──────────────────────────────── */}
      {presets.length > 0 && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '0 14px', height: 36, backgroundColor: BG_HEADER, borderTop: `2px solid ${GOLD}`, flexShrink: 0, overflow: 'hidden' }}>
          <span style={{ fontSize: 10, fontWeight: 'bold', color: '#666', textTransform: 'uppercase', letterSpacing: '0.08em', flexShrink: 0 }}>Presets</span>
          <div style={{ width: 1, height: 14, background: '#333', flexShrink: 0 }} />
          <div style={{ display: 'flex', gap: 5, overflow: 'hidden', alignItems: 'center' }}>
            {presets.filter(p => p.hotkey !== undefined && p.id !== '__last_session__').slice(0, 9).map((p, idx) => (
              <button key={p.id} title={`Load: ${p.name}`}
                style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '0 7px', height: 22, backgroundColor: '#1a1a1a', border: `1px solid #333`, borderRadius: 4, color: '#aaa', fontSize: 10, fontWeight: 'bold', cursor: 'pointer', fontFamily: FONT, transition: 'all 0.15s', whiteSpace: 'nowrap', flexShrink: 0 }}
                onMouseEnter={e => { e.currentTarget.style.borderColor = GOLD; e.currentTarget.style.color = GOLD; }}
                onMouseLeave={e => { e.currentTarget.style.borderColor = '#333'; e.currentTarget.style.color = '#aaa'; }}
              >
                <span style={{ background: GOLD, color: 'black', borderRadius: 2, width: 13, height: 13, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 9, fontWeight: 'bold', flexShrink: 0 }}>{idx + 1}</span>
                <span style={{ overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: 100 }}>{p.name}</span>
              </button>
            ))}
          </div>
          <div style={{ flex: 1 }} />
          <span style={{ fontSize: 10, color: '#555', flexShrink: 0 }}>Keys 1–9 to load</span>
        </div>
      )}
